...
const (
	...
	FLAG_PARENT_CHAIN_VALIDATOR_INFOS_CHECK_INTERVAL     = "parent-chain-validator-infos-check-interval"
		...
		Commands: []*cli.Command{
			{
				...
				Flags: []cli.Flag{
					...
					&cli.DurationFlag{
						Name:    FLAG_PARENT_CHAIN_VALIDATOR_INFOS_CHECK_INTERVAL,
						Usage:   "How often each validator infos must be checked",
						Value:   time.Minute,
						EnvVars: []string{"PARENT_CHAIN_VALIDATOR_INFOS_CHECK_INTERVAL"},
					},
					&cli.StringFlag{
...
func startParentChainJobs(ep *ParentChainEndpoint, ctx *cli.Context) {
	...
	validatorInfoCollector = NewValidatorInfoCollector(ep)
	StartJob("validator-info", network, validatorInfoCollector.collectValidatorsInfos, ctx.Duration(FLAG_PARENT_CHAIN_VALIDATOR_INFOS_CHECK_INTERVAL))
}
